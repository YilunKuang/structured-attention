...
import in_context_regression.models
from in_context_regression.samplers import get_data_sampler, sample_transformation
...
def build_evals(conf):
    n_dims = conf.model_n_dims
    n_points = conf.training_curriculum_points_end
    batch_size = conf.training_batch_size

    task_name = conf.training_task
...
def get_run_metrics_new(
    run_path, model=None, step=-1, cache=True, skip_model_load=False, skip_baselines=False
):
    if skip_model_load:
        _, conf = get_model_from_run(run_path, only_conf=True)
        all_models = []
        ...
        all_models = [model]
        if not skip_baselines:
            all_models += models.get_relevant_baselines(conf.training_task)
    evaluation_kwargs = build_evals(conf)
